...
public class Modal_Equip : MonoBehaviour
{
    [SerializeField] private Image itemIcon;
    [SerializeField] private TextMeshProUGUI itemRank;
    [SerializeField] private TextMeshProUGUI itemName;
    [SerializeField] private TextMeshProUGUI itemEquipable;
    [SerializeField] private TextMeshProUGUI itemDesc;
    [SerializeField] private TextMeshProUGUI[] itemStatus;

    private string[] strStatus = new string[5] {"체력", "공격력", "공격 딜레이", "방어력", "사거리"};

    public void SetModal(Sprite image, int rank, string name, bool equipable, string desc, float[] status)
    {
        itemIcon.sprite = image;
        SetTextRank(rank);
        itemName.text = name;
        itemDesc.text = desc;
        SetTextEquipable(equipable);
    ...
    private void SetTextEquipable(bool equipable)
    {
        if (equipable)
        {
            itemEquipable.color = Color.white;
            itemEquipable.text = "장착 가능";
        }
        else
        {
            itemEquipable.color = Color.red;
            itemEquipable.text = "장착 불가";
        }
    }
